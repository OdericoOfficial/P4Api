...
class CMSController(object):
    ...
    def __init__(self, sw_name, set_hash, set_table):
    ...
    #def set_qt(self):
        #len = eval(input("please input the length of queue: "))
        #self.controller.table_add("qt", "set_threshold", ['0'], [str(len)])
    ...
    def decode_registers(self, eps, n, mod, ground_truth_file="sent_flows.pickle"):
        ...
        for flow, n_packets in flows.items():
            ...
            msg = "Packets sent and read by the cms: {}/{}".format(n_packets, cms);
            blocks.append(cms)
            print(msg)
            if not (cms <(n_packets + (eps*n))):
                confidence_count +=1
        p = float(confidence_count)/len(flows)*100
        msg = "Not hold for {}%".format(p)
        blocks.append(p)
        print(msg)
        with open("/mnt/vir/cms.json","w") as fw:
            result = '[' + str(float(blocks[0])) + ',' + str(float(blocks[1])) + ',' + str(blocks[2]) + ']'
            fw.write(result)
        return blocks

def Decode():
    controller = CMSController('s1', False, False)
    return controller.decode_registers(0.1, 1000, 28, 'sent_flows.pickle')

def Reset():
    controller = CMSController('s1', True, True)
...
if __name__ == "__main__":
    ...
    #parser.add_argument('--table', help="add table true or false", type=str, required=False, default="false")
